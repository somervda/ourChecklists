...
This project aims to provide a starting point for angular applications. Intent is for designer of a new application to take a copy of this project, perform a set of standard replace operations to rename the project to the required new name and then start adding application specific code. The ourChecklists application provides the following capabilities that can be extended by the application designer. See application at https://ourChecklists.firebaseapp.com/

**ourChecklists Capabilities**

- Angular 9+ (Version updated as new major releases of Angular occurs)
- Firebase - ourChecklists includes firestore, authentication, hosting, firestore rules and firebase functions
...
- E2E test suite - Cypress test suite included to test ourChecklists capabilities and can be extended for applications based on ourChecklists (See section below for more details on e2e functionality)
- Built/tested/optimized - Base ourChecklists capabilities are built in, tested, and optimized to ensure new applications start at a consistent and well architected starting point.

## Creating a new new project based on ourChecklists

**Getting a copy of ourChecklists Project**

A new project can be created based on the ourChecklists code. To do this will require a new firebase instance (See instructions below "Create Initial Firebase Project"). Note: these instructions do not include detailed instructions for firebase administration or how to manage your new project in github (You are on your own for that).

Create copy from the github template (GitHub repository created)

- Go to https://github.com/somervda/ourChecklists/generate and create a new github project based on the ourChecklists project.
...
- Get a zip copy of the ourChecklists code from github https://github.com/somervda/ourChecklists/archive/master.zip
- Open the zip file and copy the content of the top level ourChecklists-master folder in the zip to the new project folder.
...
1. Perform a File->Replace in Files in VSCode. Replace "ourChecklists" to "[New Project Name]" and "our-checklists" to "[New Project Name without capitals format]" e.g. in the case of creating a new project called ourDars , ourChecklists->ourDars , our-checklists->our-dars .
...
The previous instructions will set up a direct copy of the ourChecklists code with your new project name, application specific changes will still be needed before adding your own code.

1. Use your own firebase project information(Please don't use the ourChecklists defaults, I will be periodically refreshing this firebase project to make sure it stays fresh. ). The firebase config information is stored in the src/environments folder files (environment.prod.ts and environment.ts).
...
The Cypress tooling (https://www.cypress.io/) is used to provide end to end testing for the ourChecklists application and any applications based on ourChecklists.
